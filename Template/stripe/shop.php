				...
				<div id="store" class="col-md-9">
						...
						if(isset($_POST['add_to_cart'])){
							...
							if(mysqli_num_rows($select_cart) > 0){
							   ...
							   $insert_product = mysqli_query($conn, "INSERT INTO `cart`(lap_id,person,brand,name, price, image, quantity,stock) VALUES('$lap_id','$a','$product_brand','$product_name', '$product_price', '$product_image', '$product_quantity','$product_stock')");
...
if(isset($message)){
   foreach($message as $message){
      echo '<div class="message"><span>'.$message.'</span> <i class="" onclick="this.parentElement.style.display = `none`;"></i> </div>';
   };
};
...
if(isset($_POST['buy_now'])){
    ...
    $select_cart = mysqli_query($conn, "SELECT * FROM `purchase` WHERE name = '$product_name' && person ='$a'");
 
    if(mysqli_num_rows($select_cart) > 0){
       $message[] = 'Product already purchased';
    }else{
       $insert_product = mysqli_query($conn, "INSERT INTO `purchase`(lap_id,person,brand,name, price, image, quantity) VALUES('$lap_id','$a','$product_brand','$product_name', '$product_price', '$product_image', '$product_quantity')");
     $_SESSION['lap_name']=$product_name;
    ?><script>
 ...
 if(isset($message)){
    foreach($message as $message){
       echo '<div class="message"><span>'.$message.'</span> <i class="" onclick="this.parentElement.style.display = `none`;"></i> </div>';
                        ...
                        <?php $select_products = mysqli_query($conn, "SELECT * FROM `laptops`");
						...
						<!-- store products -->
							...
							<div class=" col-xs-6">
								<div class="product" >
									...
									<input type="hidden" name="product_brand" id="product_brand" value="<?php echo $row['brand']; ?>">
                                    ...
                                    <input type="hidden" name="product_stock" value="<?php echo $row['stock']; ?>">
                                        ...
                                        <button type="submit" value="add to cart" name="buy_now" class="add-to-cart-btn"><i class="fa fa-shopping-cart"></i> buy now</button>
                                    </div>
								</div>
							</div>
										</form>
