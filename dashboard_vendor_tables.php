...
if(isset($_POST['update_update_btn'])){
   $update_value = $_POST['update_stock'];
   $update_id = $_POST['update_stock_id'];
   $update_stock_query = mysqli_query($conn, "UPDATE `laptops` SET stock = '$update_value' WHERE id = '$update_id'");
   if($update_stock_query){
      ?><script>window.locatio.href='dashboard_vendor_tables.php';</script>
      <?php
   };
};
?>
            ...
            <div class="card-body">
              ...
              <table class="table table-hover" id="table">
                ...
                <tbody>
                  <tr>
                    <?php
            $query="select * from laptops where vendor = '$b' ORDER BY stock desc";
            $result=mysqli_query($conn,$query);
                ...
                $_SESSION['id']=$row['id'];
                      ...
                      <td>
                            <form action="" method="post">
                                ...
                                <?php if($row['stock']<1) { ?>
                                <input style="width: 50px; height: 30px;background:red;color : white" type="number" name="update_stock" min="1"  value="<?php echo $row['stock']; ?>" >
                                <?php } else { ?>
                                <input style="width: 50px; height: 30px;background: #9acd4600;color : white" type="number" name="update_stock" min="1"  value="<?php echo $row['stock']; ?>" >
                                <?php } ?>
                            </td>        
                       <td> <button  value="update"  class="button-7" name="update_update_btn">Update</button></td> 
                      </form>   
                    </tr>
                    </tr>
                    <?php
