                ...
                <div class="collapse navbar-collapse" id="navbarNav">
                    <ul class="navbar-nav align-items-lg-center ms-auto me-lg-5">
                        ...
                        <div class="dropdown " style="margin-right:65px;">
                            <a href="cart.php"><img src="./images/cart.png " height="50" width="50" /></a>
                            <div class="qty"><?php echo $row_count; ?></div>
                        ...
                        ?>
                            <div class="dropdown " style="margin-right:-111px;">
                                <a class="dropdown-toggle d-flex align-items-center hidden-arrow" style="color: white" href="#" id="navbarDropdownMenuAvatar" role="button" data-mdb-toggle="dropdown" aria-expanded="false">
                                    <img src="./uploads/<?php echo $row['file']; ?>" width="50" height="50" class="rounded-circle" alt="profile pic" loading="lazy" />
                                </a>
        ...
        <i class="fa fa-search"></i>
            ...
            <div id="store" class="col-md-9">
                ...
                if (isset($_POST['compare'])) {
                    ...
                    $select_cart = mysqli_query($conn, "SELECT * FROM `compare` WHERE person ='$a'");
                    $num_rows = mysqli_num_rows($select_cart);
                    if ($num_rows < 3) {
                        // Insert a new row
                        $insert_product = mysqli_query($conn, "INSERT INTO `compare`(lap_id,person, image,name,processor,ram,storage,display,price) VALUES('$lap_id','$a', '$product_image','$product_name','$product_processor', '$product_ram', '$product_storage', '$product_display', '$product_price')");
                ?><script>
                            window.location.href = 'compare.php';
                        </script><?php
                                } else {
                                    // Update an existing row
                                    $row_to_update = $num_rows - 3; // Update the oldest of the three rows
                                    $row_query = mysqli_query($conn, "SELECT * FROM `compare` WHERE person ='$a' LIMIT 1 OFFSET $row_to_update");
                                    $row = mysqli_fetch_assoc($row_query);
                                    $row_id = $row['id'];
                                    $update_product = mysqli_query($conn, "UPDATE `compare` SET lap_id='$lap_id',person = '$a', image='$product_image',name='$product_name',processor='$product_processor',ram='$product_ram',storage='$product_storage',display='$product_display',price ='$product_price' WHERE id='$row_id'");
                                    ?><script>
                            window.location.href = 'compare.php';
                        </script><?php
                                }
                            ...
                            if (isset($_POST["view"])) {
                                ...
                                // Redirect to view.php page with the lap_id passed as a query parameter using JavaScript
                                    ?>
                    <script>
                        window.location.href = 'view.php?product_id=<?php echo $lap_id; ?>';
                    </script>
                    <?php
                                exit();
                            }
                            if (isset($_POST['wish'])) {
                                ...
                                $select_cart = mysqli_query($conn, "SELECT * FROM `wishlist` WHERE name = '$product_name' && person ='$a'");
                    ...
                    <?php
                                    ...
                                    $insert_product = mysqli_query($conn, "INSERT INTO `wishlist`(lap_id,person, image,name,processor,ram,storage,display,price) VALUES('$lap_id','$a', '$product_image','$product_name','$product_processor', '$product_ram', '$product_storage', '$product_display', '$product_price')");
