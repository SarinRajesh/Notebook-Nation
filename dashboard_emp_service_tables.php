...
if(isset($_POST['update_update_btn'])){
   ...
   $update_stock_query = mysqli_query($conn, "UPDATE `laptops` SET price = '$update_value' WHERE id = '$update_id'");
   if($update_stock_query){
      ?><script>window.locatio.href='dashboard_employee_tables.php';</script>
      <?php
   };
};
?>
                ...
                <tbody>
                  ...
                  while($row = mysqli_fetch_array($result)){
                    ...
                    <?php echo $row['address'];?></td>
                     ...
                     Laptop id : <?php echo $row['id'];?></td>
                     <td><textarea  cols="18" rows="5"><?php echo $row['complaint'];?></textarea></td>
                           
                   <?php if($row['status']=='Not confirmed') { ?>
                     <td style="color:skyblue"><?php echo $row['status'];?> </td>          
                      <td><button class="btn btn-success"><?php echo "<a href='dashboard_service_confirm.php?id=$row[id]'>Confirm</a>"?></button>
                   <button class="btn btn-danger"><?php echo "<a href='dashboard_service_reject.php?id=$row[id]'>Reject</a>"?></button></td>
                  
                   <?php } else if($row['status']=='Confirmed') { ?>
                     <td style="color:lightgreen"><?php echo $row['status'];?></td>           
                      <td><button class="btn btn-success" disabled><?php echo "<a href='dashboard_service_status.php?id=$row[id]'>Confirmed</a>"?></button></td>
                      <?php } else if($row['status']=='Serviced') { ?>
                     <td style="color:lightgreen"><?php echo $row['status'];?></td>           
                      <td><button class="btn btn-success" disabled><?php echo "<a href='dashboard_service_status.php?id=$row[id]'>Confirmed</a>"?></button></td>
                      <?php } else { ?> 
                        <td style="color:red"><?php echo $row['status'];?>  </td>
                  <td> <button class="btn btn-danger" disabled><?php echo "<a href='dashboard_service_status.php?id=$row[id]'>Rejected</a>"?></button></td>
                 <?php } ?>
                    </tr>
                    <?php
                    }
